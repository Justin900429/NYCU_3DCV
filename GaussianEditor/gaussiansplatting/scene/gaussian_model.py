...
class GaussianModel:
    ...
    def get_near_gaussians_by_mask(self, mask, dist_thresh: float = 0.1):
        ...
        bbox_3D = torch.stack(
            [
                torch.quantile(object_xyz[:, 0], 0.03),
                torch.quantile(object_xyz[:, 0], 0.97),
                torch.quantile(object_xyz[:, 1], 0.03),
                torch.quantile(object_xyz[:, 1], 0.97),
                torch.quantile(object_xyz[:, 2], 0.03),
                torch.quantile(object_xyz[:, 2], 0.97),
            ]
        )
        ...
        in_bbox = (
            (remaining_xyz[:, 0] >= bbox_3D[0])
            & (remaining_xyz[:, 0] <= bbox_3D[1])
            & (remaining_xyz[:, 1] >= bbox_3D[2])
            & (remaining_xyz[:, 1] <= bbox_3D[3])
            & (remaining_xyz[:, 2] >= bbox_3D[4])
            & (remaining_xyz[:, 2] <= bbox_3D[5])
        )
    ...
    def concat_gaussians(self, another_gaussian):
        ...
        self.mask = ~self.mask
        self.mask = torch.cat(
            [self.mask, torch.ones_like(new_opacities[:, 0], dtype=torch.bool)], dim=0
        )
        self.remove_grad_mask()
        self.apply_grad_mask(self.mask)

        self._generation = torch.cat(
            [
                self._generation,
                torch.zeros_like(new_opacities[:, 0], dtype=torch.int64),
            ],
            dim=0,
        )
        self.update_anchor()
