...
class HierarchicalGaussianModel(GaussianModel):
    ...
    def training_setup(self, training_args):
        ...
        self.xyz_gradient_accum = torch.zeros(
            (self.get_target_xyz.shape[0], 1), device="cuda"
        )
        self.denom = torch.zeros((self.get_target_xyz.shape[0], 1), device="cuda")

        l = [
            {
                "params": [self.target_xyz],
                "lr": training_args.position_lr_init * self.spatial_lr_scale,
                "name": "xyz",
            },
            {
                "params": [self.target_features_dc],
                "lr": training_args.feature_lr,
                "name": "f_dc",
            },
            {
                "params": [self.target_features_rest],
                "lr": training_args.feature_lr / 20.0,
                "name": "f_rest",
            },
            {
                "params": [self.target_opacity],
                "lr": training_args.opacity_lr,
                "name": "opacity",
            },
            {
                "params": [self.target_scaling],
                "lr": training_args.scaling_lr,
                "name": "scaling",
            },
            {
                "params": [self.target_rotation],
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(
            torch.min(
                self.get_target_opacity, torch.ones_like(self.get_target_opacity) * 0.01
            )
        )
    ...
    def load_ply(self, path):
        ...
        self.target_xyz = nn.Parameter(
            torch.tensor(
                xyz[semantic_mask], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self.target_features_dc = nn.Parameter(
            torch.tensor(features_dc[semantic_mask], dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
        self.target_features_rest = nn.Parameter(
            torch.tensor(
                features_extra[semantic_mask], dtype=torch.float, device="cuda"
            )
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
        self.target_opacity = nn.Parameter(
            torch.tensor(
                opacities[semantic_mask], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self.target_scaling = nn.Parameter(
            torch.tensor(
                scales[semantic_mask], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self.target_rotation = nn.Parameter(
            torch.tensor(
                rots[semantic_mask], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )

        self.bkg_xyz = torch.tensor(
            xyz[~semantic_mask], dtype=torch.float, device="cuda"
        )
        self.bkg_features_dc = (
            torch.tensor(features_dc[~semantic_mask], dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
        )
        self.bkg_features_rest = (
            torch.tensor(
                features_extra[~semantic_mask], dtype=torch.float, device="cuda"
            )
            .transpose(1, 2)
        ...
        self.bkg_opacity = torch.tensor(
            opacities[~semantic_mask], dtype=torch.float, device="cuda"
        )
        self.bkg_scaling = torch.tensor(
            scales[~semantic_mask], dtype=torch.float, device="cuda"
        )
        self.bkg_rotation = torch.tensor(
            rots[~semantic_mask], dtype=torch.float, device="cuda"
        )
    ...
    def update_whole_scene_property(self):
        ...
        self._features_dc = torch.concatenate(
            [self.bkg_features_dc, self.target_features_dc], dim=0
        )
        self._features_rest = torch.concatenate(
            [self.bkg_features_rest, self.target_features_rest], dim=0
        )
        self._opacity = torch.concatenate(
            [self.bkg_opacity, self.target_opacity], dim=0
        )
        self._scaling = torch.concatenate(
            [self.bkg_scaling, self.target_scaling], dim=0
        )
        self._rotation = torch.concatenate(
            [self.bkg_rotation, self.target_rotation], dim=0
        )
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.logical_and(
            selected_pts_mask,
            torch.max(self.get_target_scaling, dim=1).values
        ...
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(
            -1
        ) + self.get_target_xyz[selected_pts_mask].repeat(N, 1)
        new_scaling = self.scaling_inverse_activation(
            self.get_target_scaling[selected_pts_mask].repeat(N, 1) / (0.8 * N)
        )
