...
from gaussiansplatting.scene.vanilla_gaussian_model import (
    GaussianModel as Vanilla_GaussianModel,
)
...
def training(
    ...
    if "n2n" in args.guide:
        ...
        pix2pix = InstructPix2PixGuidance(
    ...
    for iteration in trange(opt.iterations + 1):
        ...
        if iteration < coarse_iteration:
            ...
            if "n2n" in args.guide:
                if cur_index not in edit_frames or (iteration % per_editing_step == 0):
                    result = pix2pix(
            ...
            if "anchor" in args.guide:
                ...
                loss += (
                    0 * anchor_out["loss_anchor_color"]
                    + 5 * anchor_out["loss_anchor_geo"]
                    + 0 * anchor_out["loss_anchor_opacity"]
                    + 5 * anchor_out["loss_anchor_scale"]
                )
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations - 300:
                ...
                if (
                    ...
                    size_threshold = 20 if iteration > coarse_iteration else None
...
if __name__ == "__main__":
    ...
    cams, images, camera_extent = render_multiview_images_from_mesh(
        args.mesh, save_path=replace_filename(args.save_path, "mesh_rendering.mp4")
    )
    hori_cams, _, _ = render_multiview_images_from_mesh(
        args.mesh,
        horizontal=True,
        save_path=replace_filename(args.save_path, "horizontal_mesh_rendering.mp4"),
    )
    # hard code
