...
class GaussianEditor_Del(GaussianEditor):
    ...
    def configure(self) -> None:
        ...
        if len(self.cfg.cache_dir) > 0:
            ...
            self.cache_dir = os.path.join(
                "del_cache", self.cfg.gs_source.replace("/", "-")
            )
    ...
    def on_fit_start(self) -> None:
        ...
        assert (
            len(self.cfg.seg_prompt) > 0
        ), "please specify prompt system.seg_prompt for Delete"
        self.update_mask()
        dist_thres = (
            self.cfg.inpaint_scale * self.cameras_extent * self.gaussian.percent_dense
        )
        ...
        self.render_all_view_with_mask(
            ...
            + f"_pruned_mask_scale_{self.cfg.inpaint_scale}_{self.view_num}_view"
        )
        ...
        for i, id in tqdm(enumerate(self.view_list)):
            ...
            if not os.path.exists(cur_path) or self.cfg.cache_overwrite:
                ...
                def make_inpaint_condition(image, image_mask):
                    ...
                    image_mask = (
                        np.array(image_mask.convert("L")).astype(np.float32) / 255.0
                    )
