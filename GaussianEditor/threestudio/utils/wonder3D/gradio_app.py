...
def sam_init():
    sam_checkpoint = os.path.join(
        os.path.dirname(__file__), "sam_pt", "sam_vit_h_4b8939.pth"
    )
    model_type = "vit_h"
        ...
        with gr.Row(variant="panel"):
            with gr.Column(scale=1):
                ...
                example_folder = os.path.join(
                    os.path.dirname(__file__), "./example_images"
                )
                example_fns = [
                    os.path.join(example_folder, example)
            ...
            with gr.Column(scale=1):
                ...
                processed_image_highres = gr.Image(
                    type="pil", image_mode="RGBA", visible=False, tool=None
                )
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            output_processing = gr.CheckboxGroup(
                                ["Background Removal"],
                                label="Output Image Postprocessing",
                                value=[],
                            )
                    with gr.Row():
                        with gr.Column():
