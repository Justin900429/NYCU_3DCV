...
class BasicMVTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.joint_attention_twice:
            norm_hidden_states = (
                self.norm_joint_twice(hidden_states, timestep)
                if self.use_ada_layer_norm
                else self.norm_joint_twice(hidden_states)
            )
        ...
        if self.joint_attention:
            norm_hidden_states = (
                self.norm_joint(hidden_states, timestep)
                if self.use_ada_layer_norm
                else self.norm_joint(hidden_states)
            )
...
class XFormersMVAttnProcessor:
    ...
    def __call__(
        ...
        if multiview_attention:
            key = rearrange(
                key_raw, "(b t) d c -> b (t d) c", t=num_views
            ).repeat_interleave(num_views, dim=0)
            value = rearrange(
                value_raw, "(b t) d c -> b (t d) c", t=num_views
            ).repeat_interleave(num_views, dim=0)

            if cross_domain_attention:
                # memory efficient, cross domain attention
                key_0, key_1 = torch.chunk(
                    key_raw, dim=0, chunks=2
                )  # keys shape (b t) d c
                value_0, value_1 = torch.chunk(value_raw, dim=0, chunks=2)
                key_cross = torch.concat([key_1, key_0], dim=0)
                value_cross = torch.concat(
                    [value_1, value_0], dim=0
                )  #  shape (b t) d c
