...
class OrthoNeuSSystem(BaseSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.system.loss.geo_aware:
            normal_errors = (
                normal_errors
                * torch.exp(cosines.abs())
                / torch.exp(cosines.abs()).sum()
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # ])
        return {"psnr": 0, "index": batch["index"]}
    ...
    def export(self):
        ...
        self.save_mesh(save_path, ortho_scale=self.config.export.ortho_scale, **mesh)
