...
def dilate_mask(mask, pixels):
    kernel = torch.ones(
        1, 1, pixels * 2 + 1, pixels * 2 + 1, dtype=mask.dtype, device=mask.device
    )
    return F.max_pool2d(
        mask.unsqueeze(0), kernel_size=kernel.shape[2:], stride=1, padding=pixels
    ).squeeze(0)
...
def erode_mask(mask, pixels):
    ...
    padded_mask = F.pad(
        mask, (pixels, pixels, pixels, pixels), mode="constant", value=1
    )
    return (
        F.conv2d(padded_mask.unsqueeze(0), kernel, stride=1).squeeze(0)
        == kernel.numel()
    )
