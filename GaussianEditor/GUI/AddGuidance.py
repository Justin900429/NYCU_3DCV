...
class AddGuidance:
    ...
    def train_step(self, rendering, view_index):
        ...
        if view_index not in self.edit_frames or (
            ...
            and self.cfg.edit_begin_step < self.step < self.cfg.edit_until_step
        ...
        loss = (
            self.cfg.lambda_l1 * torch.nn.functional.l1_loss(rendering, gt_image)
            + self.cfg.lambda_p
        ...
        if (
            ...
            loss += (
                self.cfg.lambda_anchor_color * anchor_out["loss_anchor_color"]
