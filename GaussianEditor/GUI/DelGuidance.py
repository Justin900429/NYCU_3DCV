...
class DelGuidance:
    ...
    def inpaint_with_mask_ctn(self, image_in, mask_in, view_index) -> None:
        image_in_pil = to_pil_image(
            image_in[0].permute(2, 0, 1)
        )  # 1, H, W, C to C, H, W
        mask_in_pil = to_pil_image(
            torch.stack([mask_in[0].to(torch.uint8) * 255] * 3)
        )  # C, H, W 255
        ...
        self.edit_frames[view_index] = (
            self.to_tensor(out).to("cuda")[None].permute(0, 2, 3, 1)
        )  # 1 C H W to 1 H W C

        self.train_frustums[view_index].remove()
        self.train_frustums[view_index] = ui_utils.new_frustums(
            view_index,
            self.train_frames[view_index],
            self.cams[view_index],
            self.edit_frames[view_index],
            self.visible,
            self.server,
        )
    ...
    def __call__(self, rendering, image_in, mask_in, view_index, step):
        ...
        loss = (
            self.lambda_l1 * torch.nn.functional.l1_loss(rendering, gt_image)
            + self.lambda_p
            * self.perceptual_loss(
                rendering.permute(0, 3, 1, 2).contiguous(),
                gt_image.permute(0, 3, 1, 2).contiguous(),
            ).sum()
        )  # 1 H W C to 1 C H W
        # anchor loss
        if (
