...
from gaussiansplatting.gaussian_renderer import point_cloud_render, render
...
class WebUI:
    def __init__(self, cfg) -> None:
        ...
        with self.server.add_gui_folder("Semantic Tracing"):
            ...
            self.seg_cam_num = self.server.add_gui_slider(
                "Seg Camera Nums", min=1, max=200, step=1, initial_value=24
            )
            ...
            with self.server.add_gui_folder("Advanced Options"):
                ...
                with self.server.add_gui_folder("Loss Options"):
                    ...
                    self.anchor_weight_init = self.server.add_gui_slider(
                        "Anchor Init", min=0.0, max=10.0, step=0.05, initial_value=0.1
                    )
                    ...
                    self.anchor_term = [
                        self.anchor_weight_init_g0,
                        self.anchor_weight_init,
                        self.anchor_weight_multiplier,
                        self.lambda_anchor_color,
                        self.lambda_anchor_geo,
                        self.lambda_anchor_scale,
                        self.lambda_anchor_opacity,
                    ]
        ...
        def _(_):
            ...
            self.gaussian.save_ply(
                os.path.join("ui_result", "{}.ply".format(formatted_time))
            )
    ...
    def update_sam_mask_with_point_prompt(
        ...
        if self.seg_cam_num.value > 1:
            ...
            points2ds = project(self.camera, points3d)
            img = render(self.camera, self.gaussian, self.pipe, self.background_tensor)["render"]
            self.sam_predictor.set_image(
                np.asarray(to_pil_image(img.cpu())),
            )
            self.sam_features[0] = self.sam_predictor.features
            mask, _, _ = self.sam_predictor.predict(
                point_coords=points2ds.cpu().numpy(),
                point_labels=np.array([1] * points2ds.shape[0], dtype=np.int64),
                box=None,
            ...
            mask = torch.from_numpy(mask).to(torch.bool).to(get_device())
            self.gaussian.apply_weights(self.camera, weights, weights_cnt, mask.to(torch.float32))
            masks.append(mask)
        ...
        if self.guidance_type.value == "InstructPix2Pix":
            if not self.ip2p:
                ...
                self.ip2p = InstructPix2PixGuidance(
                    OmegaConf.create(
                        {"min_step_percent": 0.02, "max_step_percent": 0.98}
                    )
                )
